...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
  ...
  if is_tracking_task:
    ...
    if motion_cmd.motion_file and Path(motion_cmd.motion_file).exists():
      ...
      raise ValueError(
        ...
        "  --env.commands.motion.motion-file /path/to/motion_dir (local library)\n"
        "  --env.commands.motion.motion-file /path/to/playlist.txt (local playlist)"
      )
  ...
  if resume_path is not None:
    ...
    load_cfg = None
    if not cfg.agent.load_optimizer or not cfg.agent.load_iteration:
      load_cfg = {
        "actor": True,
        "critic": True,
        "optimizer": cfg.agent.load_optimizer,
        "iteration": cfg.agent.load_iteration,
        "rnd": True,
      }
